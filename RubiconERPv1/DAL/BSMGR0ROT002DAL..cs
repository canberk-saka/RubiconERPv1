...
{
    public class BSMGR0ROT002DAL
    {
        private readonly string _connectionString;

        public BSMGR0ROT002DAL(string connectionString)
        {
            _connectionString = connectionString; // Bağlantı dizesi başlatılır.
        }
        ...
        public void AddRecord(string comCode, string docType, string docTypeText, bool isPassive)
        {
            ...
            {
                string query = "INSERT INTO BSMGR0ROT002 (COMCODE, DOCTYPE, DOCTYPETEXT, ISPASSIVE) VALUES (@comCode, @docType, @docTypeText, @isPassive)";
        ...
        public DataTable GetAllRecords()
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                string query = "SELECT COMCODE, DOCTYPE, DOCTYPETEXT, ISPASSIVE FROM BSMGR0ROT002";
        ...
        public void DeleteRecord(string comCode, string docType)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                string query = "DELETE FROM BSMGR0ROT002 WHERE COMCODE = @comCode AND DOCTYPE = @docType";
                SqlCommand command = new SqlCommand(query, connection);
                command.Parameters.AddWithValue("@comCode", comCode);
                command.Parameters.AddWithValue("@docType", docType);
